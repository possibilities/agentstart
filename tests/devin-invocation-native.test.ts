...
import { prepareDevinInvocation } from "../scripts/devin-invocation.ts";
...
test("real Devin finds the AgentStart Role without the global default plugin", {
  ...
  try {
    ...
    prepareDevinInvocation(repo, home, resources, join(home, ".local/state/agentstart/devin-invocations"));
    prepared = true;
    ...
    const run = (args: string[], cwd = repo) => {
      const child = Bun.spawnSync([native, ...args], { cwd, env, stdout: "pipe", stderr: "pipe", timeout: 20_000 });
    ...
    expect(run(["skills", "list"], subdir)).toContain("prime");
    expect(run(["mcp", "list"], subdir)).toContain("agentbrain");
    expect(statSync(join(repo, ".devin/mcp_config.local.json")).mode & 0o777).toBe(0o600);
  } finally {
    if (prepared) cleanupDevinInvocations(join(home, ".local/state/agentstart/devin-invocations"), () => null);
