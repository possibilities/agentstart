...
class RoleRender(unittest.TestCase):
    ...
    def test_shipped_roles_are_complete_and_modes_fit_native_limit(self):
        ...
        self.assertIn("automatic comparison profile exists only when the human explicitly enables a named configuration", manager_prompt)
        self.assertIn("source and counterpart harnesses, providers, models, efforts, and repetition", manager_prompt)
        self.assertIn("do not infer unspecified variants or turn the role into a general benchmark system", manager_prompt)
        self.assertIn("`grok-4.6-counterpart`", manager_prompt)
        self.assertIn("Sol-, Terra-, or Luna-level root assignment one isolated Grok 4.6 counterpart", manager_prompt)
        self.assertIn("separate tracked Work/Assignment lineage", manager_prompt)
        self.assertIn("start both candidates from the same source state", manager_prompt)
        self.assertIn("identify the profile and source/counterpart relationship in their routing receipts", manager_prompt)
        self.assertIn("Do not mirror Astra-level judgment", manager_prompt)
        self.assertIn("shared device or headful resource", manager_prompt)
        self.assertIn("Never automatically integrate a comparison artifact", manager_prompt)
        ...
        self.assertIn("one isolated AgentFX/Grok 4.6 medium shadow", manager_prompt)
        self.assertIn("do not steer, resume, chain or integrate either comparison artifact", manager_prompt)
        self.assertIn("Stages 2 and 3 remain disabled recipes", manager_prompt)
        self.assertIn("Comparison selection does not change AgentUsage admission", manager_prompt)
