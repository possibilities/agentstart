...
function fixture() {
  ...
  writeFileSync(join(home, ".opencode", "settings.json"), '{"preserved":true}\n');
  writeFileSync(
    join(tools, "npm"),
    `#!/bin/bash
...
test("cuts over opencode and retires the verified V1 executable and owned opencode2 link", () => {
  const f = fixture();
  ...
  expect(existsSync(join(f.legacyBin, "opencode"))).toBe(false);
  expect(readFileSync(join(f.base, "home", ".opencode", "settings.json"), "utf8")).toContain("preserved");
});
...
test("refuses to retire a non-V1 executable or a symlink in the legacy path", () => {
  const newer = fixture();
  writeFileSync(join(newer.legacyBin, "opencode"), "#!/bin/bash\nprintf 'opencode v2.0.16\\n'\n", { mode: 0o755 });
  expect(newer.run(["--install"]).stderr.toString()).toContain("unexpected opencode version");
  expect(existsSync(newer.log)).toBe(false);

  const linked = fixture();
  rmSync(join(linked.legacyBin, "opencode"));
  symlinkSync(join(linked.base, "foreign"), join(linked.legacyBin, "opencode"));
  expect(linked.run(["--install"]).stderr.toString()).toContain("independent or unreadable executable");
  expect(existsSync(linked.log)).toBe(false);
});

test("retires V1 even when an older shell searches the legacy path first", () => {
  const f = fixture();
  const result = f.run(["--install"], { PATH: `${join(f.base, "tools")}:${f.legacyBin}:${f.binDir}:/usr/bin:/bin` });
  expect(result.exitCode, result.stderr.toString()).toBe(0);
  expect(existsSync(join(f.legacyBin, "opencode"))).toBe(false);
  expect(Bun.spawnSync([join(f.binDir, "opencode"), "--version"]).stdout.toString()).toBe("opencode v2.0.16\n");
});
