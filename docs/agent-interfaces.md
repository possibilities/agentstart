...
AgentNotify is a deliberate parity interface: `agentnotify` preserves terminal-notifier CLI behavior, while the native app’s private Unix socket and stdio MCP expose the same durable notification operations. AgentStart installs a `~/.local/bin/terminal-notifier` router through `install-agentlaunch-shims`, ahead of Homebrew on the fleet PATH. It checks AgentNotify availability before dispatch and falls back to the retained original notifier only before a notification is submitted. After dispatch, it preserves the result without retrying: native denial, timeout, or failure can already represent a durable item. AgentNotify’s one-time setup offer, Preferences button, and shared `installShim` operation delegate to that same `scripts/install-notification-shim` owner for explicit installation. Funk uses that managed path for notifications and dismissal. Explicit absolute or vendored upstream binaries bypass a PATH shim; ordinary callers should use the router. `AGENTSTART_TERMINAL_NOTIFIER_FALLBACK` can select an explicit original executable. `skills/notifications` in AgentNotify owns expert usage; the older `notify` skill directs callers there. AgentLaunch and AgentVoice receive it through the shared inventory. The authenticated HTTP fleet and Grok toolsets expose its full contract with the `agentnotify_` prefix, including durable appearance preferences; no separate prompt tool catalog or per-client notification registry is needed. The app starts on demand; installation never restarts a running inbox.
