...
Claude receives a session-only `agent` plugin, and Codex receives a globally
installed strictly skills-only `agent` plugin whose qualified names are
persistently disabled and session-enabled only by AgentLaunch.
...
balancing, claims, guidance, and statuslines remain.
