...
| agentstart | fmx | installs the fmx checkout editable — frozen `bun install` plus `bun link`, so `~/.bun/bin/fmx` runs `~/code/fmx/src/index.ts` live — proves its Fx pin equals AgentStart's gated Integration pin and atomically copies fxnk's source-built `fx` to the distinct regular file `~/.local/bin/fmx-fx`; then it runs fmx's `scripts/install-companion.sh`, which builds the Companion `companion.json` pins from `~/src/zmx` into `~/.local/bin/fmx-zmx` (a release ships both native dependencies beside `fmx`; an editable fmx finds them on PATH), doctors the editable triple, and links the tracked operator config into `~/.config/fmx/config.toml`; fmx's key schema stays a strict subset of Herdr's and uses the same `ctrl+space` prefix. The separately installed `fmx-release-local` helper calls Fmx's own release and PTY scripts twice in series — native arm64, then Rosetta x86_64 with pinned toolchains — combines them with the named Actions run's completed Linux artifacts, and keeps cancellation, Blob publication/public verification/latest-only pruning, and exact tagging behind an explicit `publish --cancel-run` boundary | `agentstart/scripts/install.sh` (fmx block); `agentstart/scripts/fmx-release-local`; `fmx/scripts/build-release.sh`; `fmx/tests/multiplexer.e2e.test.ts`; `fmx/scripts/install-companion.sh`; `agentstart/config/fmx/config.toml`; `agentstart/scripts/fmx-config`; asserted by `agentstart/tests/validate.sh`, `agentstart/tests/fmx-release-local.sh`, and `agentstart/tests/fmx-config.sh` |
