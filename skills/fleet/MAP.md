...
flowchart LR
    ...
    executor -->|stdio tools generated from agent contracts| contractServers[agentattention / agentboard / agentwiki / agentbrain / agentsearch / agentscrape / agentkeys / agentbrowse / agentgrok / agentsounds / agentchats]
...
| Executor / other MCP hosts | agentattention / agentboard / agentwiki / agentbrain / agentsearch / agentscrape / agentkeys / agentbrowse / agentgrok / agentsounds | starts `<cli> mcp` over stdio and receives tools generated from that CLI's own agent contract. Each server dispatches through its command table in process; only `audience: agent` leaves are exposed. Managed Claude/Codex workflows discover and call these tools through Executor. Success and domain-error envelopes are preserved as structured content and standalone JSON text, including Executor's error content. Transport shutdown closes the owned stdio server | `agentstart/config/agent-contract/MCP.md`; `agentstart/config/executor/integrations.json`; each repository's MCP modules and handshake tests (`src/` except AgentBrowse's `cli/`) |
| Executor | agentsounds | serves `notify` and `guide` through the same typed handlers as the CLI. MCP preserves explicit flag presence, requires absolute recipe/export paths, and cancels and reaps active playback on cancellation or transport shutdown. The human audition TUI and operator hooks remain available | `agentsounds/src/commands.ts`; `agentsounds/src/mcp-tools.ts`; `agentsounds/src/mcp-server.ts`; `agentsounds/src/mcp.ts`; `agentsounds/test/mcp.test.ts` |
...
browser, and grokbot now teach Executor discovery and structured calls, with
