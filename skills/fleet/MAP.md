...
| AgentNotify setup / Preferences / CLI / MCP | AgentStart notification shim installer | calls | `NotifyCore/NotificationShim.swift` invokes `~/code/agentstart/scripts/install-notification-shim` only for explicit installation, installing the same router into `~/.local/bin`. The app remembers its one-time offer independently of task state; disposable services use an isolated destination. Missing AgentStart is reported without writing another implementation. |
| AgentStart notification router | AgentNotify, original terminal-notifier | calls | `config/notifications/terminal-notifier` probes `agentnotify diagnose` with a 12-second bound, then execs one backend with unchanged argv/stdin. `scripts/install-agentlaunch-shims` invokes the internal ownership-checked `install-notification-shim`; no Homebrew binary is overwritten. `AGENTSTART_TERMINAL_NOTIFIER_FALLBACK` selects an explicit fallback. |
| Funk notification helpers | AgentStart notification router | calls | `funk/bin/.local/bin/funk-notify`, `funk/libexec/funk-update`, and `funk/libexec/verify-notifications` prefer `~/.local/bin/terminal-notifier`. Diagnosis verifies AgentNotify's required own-arrivals-only policy without creating a durable probe task; original-notifier fallback diagnosis remains available. The retired bulk-dismiss helper and its `ctrl+cmd+escape` skhd binding no longer consume AgentNotify's Complete All chord. |
...
| Fleet building/delivery guidance | notifications skill | routes | `prompts/agentguidance/GUIDELINES.md` prefers AgentNotify’s `skills/notifications/SKILL.md` for durable completion and attention notices. The older `notify` entrypoint routes to that skill and explains the AgentStart availability fallback. |

AgentNotify’s app owns its private account-local Unix socket. No AgentStart LaunchAgent is installed for it; CLI/MCP launches the app on demand. The managed `~/.local/bin/terminal-notifier` router redirects PATH callers through AgentNotify. Funk explicitly prefers that path even when its caller has Homebrew first. The original Homebrew notifier remains installed and is used if the AgentNotify executable or its read-only readiness check is unavailable. Once a request is dispatched, the router preserves its output/status and never replays it through the fallback. Absolute and vendored binaries bypass PATH routing. AgentNotify accepts into its durable store and presents only through its own inbox and sticky arrival preview; it never requests system-notification authorization or posts macOS banners, sounds, categories, or Notification Center entries.
