...
| agentstack | codexnk | every `server_start` launches `~/.local/libexec/codexnk/codex`; no executable request field, PATH lookup, or vendor fallback. The runtime is a required setup dependency; a legacy live ID using a different binary must be explicitly stopped before reuse. AgentStack signs in through that runtime, keeps account credentials in private SQLite, and passes `--identity`, `--capabilities`, and `--history-dir` for new managed Servers. It locates codexnk's retained runtime using a per-Server private `TMPDIR` and best-effort reconciles refreshed `auth.json` back into SQLite without modifying the fork. A Server retains its launch account when the active choice changes | `agentstack/packages/codex/src/login.ts`; `agentstack/packages/codex/src/store.ts`; `agentstack/packages/codex/src/supervisor.ts`; `agentstack/packages/codex/src/runtime-auth.ts`; `agentstack/docs/adr/0003-required-codexnk-runtime.md`; `agentstack/docs/adr/0004-codex-account-state.md` |
...
| agentroles | Codex | delivers one explicit role through native `-c` model-instruction and MCP overrides and an installed role plugin; the bare shim adds only the default permission mode | `agentroles/src/deliver/codex.ts`; `agentroles/src/codex-plugin.ts`; `agentroles/src/render.ts` |
...
| agentroles | Fx | delivers prompt, MCP and skill paths for one invocation through native Fx flags; no global fleet overlay is applied | `agentroles/src/deliver/fx.ts`; `agentroles/src/main.ts`; `agentroles/README.md` |
...
| Explicit default role / AgentVoice | individual stdio MCP servers | the rendered `default` role omits AgentAttention, AgentChats, AgentGrok, AgentHUD, AgentKeys, AgentMux, AgentSounds, and AgentSurface while retaining its HUD skill and installed commands; bare Claude/Codex permission shims inject no fleet inventory | `agentstart/config/resources/mcp-servers.json`; `agentstart/roles/default/mcp.json`; `agentstart/scripts/render-roles`; `agentstart/scripts/install-harness-shims`; `agentstart/docs/adr/0040-collapse-explicit-roles-to-default.md` |
...
| AgentStart default role | agentchats | Use `routing-receipt` through the documented CLI to validate bounded decision/acceptance tool output; the role does not start AgentChats MCP, and no new store or service is added. `routing` reads exact native transcripts to join the evidence. Missing capability preserves a capture gap without blocking work. | `agentstart/roles/default/APPEND_SYSTEM_PROMPT.md`; `agentchats/src/routing/`; `agentchats/src/cli/contract.ts` |
