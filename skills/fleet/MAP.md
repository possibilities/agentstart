...
| Explicit roles / AgentVoice | individual stdio MCP servers | explicit manager and worker roles receive their rendered inventory; bare Claude/Codex shims inject none | `agentstart/config/resources/mcp-servers.json`; `agentstart/scripts/render-roles`; `agentstart/scripts/install-harness-shims` |
