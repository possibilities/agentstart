...
| agentstart | fmx | installs the fmx checkout editable — frozen `bun install` plus `bun link`, so `~/.bun/bin/fmx` runs `~/code/fmx/src/index.ts` live — proves its Fx pin equals AgentStart's gated Integration pin and atomically copies fxnk's source-built `fx` to the distinct regular file `~/.local/bin/fmx-fx`; then it runs fmx's `scripts/install-companion.sh`, which builds the Companion `companion.json` pins from `~/src/zmx` into `~/.local/bin/fmx-zmx` (a release ships both native dependencies beside `fmx`; an editable fmx finds them on PATH), doctors the editable triple, and links the tracked operator config into `~/.config/fmx/config.toml`; fmx's key schema stays a strict subset of Herdr's and uses the same `ctrl+space` prefix | `agentstart/scripts/install.sh` (fmx block); `fmx/scripts/install-companion.sh`; `agentstart/config/fmx/config.toml`; `agentstart/scripts/fmx-config`; asserted by `agentstart/tests/validate.sh` and `agentstart/tests/fmx-config.sh` |
