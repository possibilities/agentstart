...
flowchart LR
    ...
    start -.->|config/launchd + install-launchagents| services[agentbrain worker + share + doctor / agentusage observer / agentattention server / agentscrape queue-processor / agentsource receiver + notifier / agentwiki server]
```
...
resident Agentsource service, `agentsource.notifier`, which subscribes to the
