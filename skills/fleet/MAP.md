...
flowchart LR
    ...
    lab -->|server-configured WebSocket over its dedicated Unix socket; initialize then paginated model/list and conversation RPCs| codex
...
flowchart LR
    ...
    start ==>|checkout contracts| fleet[agentwiki / archival agentboard / agentbrowse / agentattention / agentutils / agentsearch / agentkeys / agentsource / agentscrape / agentbrain / agentusage / agentlaunch / agentsurface / agentgrok / agentchats / agentroles]
    start ==>|AgentHUD-owned no-service installer| hudInstall[agenthud]
    start ==>|AgentLab-owned command/build installer| labInstall[agentlab]
    ...
    start -.->|config/launchd + install-launchagents| services[agentbrain work + share + doctor / agentusage observe / agentattention serve / agenthud serve / agentlab codex-app-server + fx-broker + serve / agentscrape process-queue / agentsource receive + notify / agentwiki serve]
```
...
| agentstart | agentlab | `install-agent-clis` invokes AgentLab's hardened `scripts/install.sh --install` contract: frozen npm dependencies, cumulative client/server build, ownership-checked editable `agentlab` command, and deployed-SHA receipt without a service action. AgentStart separately owns and orders the dedicated `io.arthack.agentlab.codex-app-server` and `io.arthack.agentlab.fx-broker` before `io.arthack.agentlab.serve`, rendering the daemons' exact installed `unix://` endpoints only into the server environment. Socket overrides select desired state only during the matching daemon convergence; console-only convergence and status observe both owned installed plists. | `agentstart/scripts/install-agent-clis`; `agentstart/config/launchd/io.arthack.agentlab.{codex-app-server,fx-broker,serve}.plist`; `agentstart/scripts/install-launchagents`; `agentlab/scripts/install.sh`; `agentlab/scripts/install.mjs`; `agentstart/docs/adr/{0033-supervise-agentlab-cumulative-service,0034-dedicated-agentlab-codex-app-server,0038-supervise-agentlab-fx-broker}.md` |
| agentlab | Codex | The server reads only AgentStart's `AGENTLAB_CODEX_ENDPOINT`, upgrades WebSocket over the dedicated Unix socket, sends stock `initialize`/`initialized`, then uses paginated `model/list` and conversation RPCs. The browser cannot supply or read the endpoint, and AgentVoice's process is outside the registry. | `agentstart/config/launchd/io.arthack.agentlab.serve.plist`; `agentlab/apps/console/lib/server/runtime.ts`; `agentlab/packages/harness-codex/src/codex-app-server-adapter.ts`; `agentlab/packages/harness-codex/tests/catalog.test.ts` |
| agentlab | Fx | The AgentStart-supervised `agentlab fx-broker` owns Fx ACP children behind a private Unix listener, publishes one generation identity and bounded per-session cursor journal, and snapshots `fx models --json` for pre-session catalog selection. Console adapters open fresh leases; closing a lease retains the native session, and reattachment replays journaled notifications plus a non-destructive snapshot without native restore or prompt retry. Broker restart changes identity because sessions were lost. No browser endpoint, credential mutation, provider prompt, fxnk patch, or AgentVoice process participates. | `agentlab/scripts/fx-broker.mjs`; `agentlab/packages/harness-fx/src/broker-client.ts`; `agentlab/packages/harness-fx/tests/broker.test.ts`; `agentlab/docs/decisions/0016-own-the-persistent-fx-attachment-broker.md`; `agentstart/config/launchd/io.arthack.agentlab.fx-broker.plist`; `agentstart/docs/adr/0038-supervise-agentlab-fx-broker.md` |
...
| agentroles | Claude Code | delivers a role directory to one `claude` invocation through PATH (so the AgentLaunch shim still applies): `--system-prompt-file` or `--append-system-prompt-file`, `--mcp-config <role>/mcp.json`, and `--plugin-dir` on a skills-only plugin rendered fresh under `~/.cache/agentroles/claude/<role>/`. The child also receives `AGENTROLES_ROLE`/`AGENTROLES_NAME`; an immediate `AGENTLAUNCH_ROLE_RESOURCES=agentroles-v1` marker tells the shim to retain the complete role resource layer instead of adding global fleet resources. AgentLaunch consumes the marker before native spawn; sticky role context alone does not suppress later default launches. The harness exit code is returned unchanged | `agentroles/src/main.ts` (`deliver`); `agentroles/src/deliver/claude.ts`; `agentroles/src/render.ts`; `agentroles/src/exec.ts`; `agentroles/README.md` |
...
| Claude Code / Codex / AgentVoice | individual stdio MCP servers | default managed AgentLaunch sessions and manager roles receive direct fleet tools, Agentdesk, agent-browser, termctrl, two account-bound Gog servers, and fleet shadcn from the same rendered manager-oriented inventory; explicit worker roles omit HUD MCP and filter the HUD skill. Native tool discovery retains each server schema and structured result. Utility invocations do not receive fleet resources | agentstart/config/resources/mcp-servers.json; agentstart/scripts/render-mcp-resources; agentstart/scripts/render-capabilities; agentlaunch/src/resources.ts; agentlaunch/test/resources.test.ts; agentstart/scripts/render-roles |
...
| agentstart | AgentVoice manager/worker roles | the fixed-resource renderer publishes canonical manager and worker directories from AgentStart-owned prompts, common portable skills with the worker's explicit exclusions, and separate MCP resources. Its ownership receipt records content-only prompt, rendered-MCP and resolved-skill hashes with AgentVoice's directory-role v1 framing. AgentVoice loads an explicitly selected role through process-local skill roots and per-thread MCP config, captures the generation's resolved content hashes, and compares them with the current directory in status; source edits do not reload a live generation | `agentstart/scripts/render-roles`; `agentstart/scripts/render-capabilities`; `agentstart/config/agentvoice/server.json`; `agentstart/tests/render-roles.py`; `agentvoice/src/core/role-content.ts`; `agentvoice/src/core/role.ts`; `agentvoice/src/core/runtime.ts` |
...
| agentlab codex-daemon | agentusage | Privately calls `prepare codex --json`, validates the v1 native args/env/unsets and loopback lease, renews by HTTP POST every 25 seconds, and releases by DELETE on exit/signals. Ambiguous preparation or renewal stops the owned native child; credentials never enter argv/logs. No AgentUsage service or credential ownership changes. | `agentlab/scripts/codex-daemon.mjs`; `agentlab/tests/codex-daemon.test.mjs`; `agentlab/docs/decisions/0015-managed-authentication-for-dedicated-codex.md` |
| agentlab codex-daemon | Codex / AgentStart shim | Starts one `codex app-server --listen unix:///...` with validated non-secret subcommand-local provider config and private lease environment; the supported `AGENTLAUNCH_SHIM_BYPASS=1` escape reaches stock Codex. Native home/history remain native; the parent owns renewal and child cleanup, and suppresses native output. AgentStart supervises the public AgentLab wrapper under the existing dedicated service label. The console remains an attachment client; AgentVoice is outside the boundary. | `agentlab/scripts/codex-daemon.mjs`; `agentstart/scripts/install-agentlaunch-shims`; `agentstart/config/launchd/io.arthack.agentlab.codex-app-server.plist`; `agentstart/docs/adr/0037-supervise-agentlab-managed-codex-launcher.md` |
...
| agentstart | herdr | installs the official stable Homebrew formula when absent and upgrades it only during an explicitly authorized socket-free maintenance run, then runs `herdr integration install claude\|codex`, links agentsurface's launcher-pane and tab-naming plugin directory with `herdr plugin link`, and renders the version-matched surface skill from `herdr --skill` into the fixed resources. While Codex integration v8 remains installed, AgentStart also converges one temporary `SessionStart` fallback at the already-trusted command path; it invokes Herdr's public `pane report-agent-session` only for AgentLaunch descendants inside Herdr and self-disables when the managed integration version advances | `agentstart/scripts/install.sh` (`install_or_upgrade_formula herdr`, `install_herdr_integrations`, `install_herdr_skill`); `agentstart/scripts/install-herdr-codex-session-fallback`; `agentstart/config/herdr/codex-session-fallback.sh`; `agentstart/scripts/herdr-socket-state`; asserted by `agentstart/tests/validate.sh` and `agentstart/tests/codex-herdr-session-fallback.py` |
...
| --- | --- | --- | --- |
| agentvoice serve | portless | Locked 0.15.6 foreground route `https://agentvoice.localhost`, editable Vite dev by default with optional production preview. The explicit `--tailscale` mode adds the exact Portless-injected tailnet-only origin while retaining localhost and refusing Funnel/ngrok; AgentStart enables it for the resident reader. Requires the existing shared loopback HTTPS proxy; no sudo or call startup. AgentStart keeps it resident as `io.arthack.agentvoice.serve`; exact-label convergence can replace the temporary submitted reader and later diagnose or converge it without operating AgentVoice's separately owned waiting server, menu app, clients, calls or future Native SDK shell. | `agentvoice/src/web-serve.ts`; `agentvoice/src/web-target.ts`; `agentvoice/web/server/{dev,preview,local-origin}.ts`; `agentvoice/web/README.md`; `agentstart/config/launchd/io.arthack.agentvoice.serve.plist`; `agentstart/scripts/install-launchagents`; `agentstart/docs/adr/0022-supervise-agentvoice-transcript-reader.md` |
...
| agentlab serve | portless | Builds the current cumulative browser UI and Node backend together, registers that same supervised process under locked Portless name `agentlab`, and serves it at `https://agentlab.localhost`; `http://agentlab.localhost` follows the shared proxy's TLS redirect. The backend remains on fixed loopback port 4177. TypeSafe environment/Keychain lookup stays server-only, the established SQLite feedback database survives browser and service restarts, and the dedicated Codex and Fx endpoints are server-only configuration. AgentStart keeps it resident as `io.arthack.agentlab.serve`; exact-label status verifies the named Jev credential state and feedback readiness routes while accepting a coherent missing-credential state. The command requires the existing shared proxy and never operates it. | `agentlab/scripts/agentlab.mjs`; `agentlab/apps/console/lib/server/runtime.ts`; `agentlab/experiments/routing-machine/server/{main,live-jev-server,feedback-store}.ts`; `agentstart/config/launchd/io.arthack.agentlab.serve.plist`; `agentstart/scripts/install-launchagents`; `agentstart/docs/adr/0033-supervise-agentlab-cumulative-service.md` |
| agentstart | agentattention, agentbrain, agentchats, agenthud, agentlab, agentvoice, agentscrape, agentsource, agentusage, agentwiki | installs their commands too. AgentChats installation retains its CLI, index, OpenTUI picker, skill, and stdio MCP without a web build or service action. AgentHUD and AgentLab are ordinary independent participants whose installers prepare their editable commands and web assets without a service action. AgentStart owns these active fleet launch agents outright: `io.arthack.agentattention.serve`, `io.arthack.agentbrain.work`, `.share`, and `.doctor`, `io.arthack.agenthud.serve`, `io.arthack.agentlab.codex-app-server`, `io.arthack.agentlab.fx-broker`, `io.arthack.agentlab.serve`, `io.arthack.agentvoice.serve`, `io.arthack.agentusage.observe`, `io.arthack.agentscrape.process-queue`, `io.arthack.agentsource.receive` and `.notify`, and `io.arthack.agentwiki.serve`. The Brain worker may retain an operator-selected `AGENTSCRAPE_BROWSER_SESSION` across installer runs, but it also sets `AGENTSCRAPE_OWN_PINNED_SESSION=1`: the serialized worker retains that Agentbrowse profile and authentication while Agentscrape closes each browser target after the operation, so an empty queue retains no browser VM. Exact-label targeting can converge or diagnose one job without operating on its neighbors; a healthy identical loaded job is not restarted. AgentVoice remains sole owner of `io.arthack.agentvoice.server`. A bounded retirement selector removes only the exact-marker-owned `io.arthack.agentchats.serve` plist and job. Labels use the account-wide `io.arthack.<project>.<verb>` grammar while the manifest separately records resident, periodic, or queue-triggered lifecycle. Ordinary plists enter through the tool's one public binary; the next row records the bounded test-checkout exception. The receiver plist names only the private secret's path, never its value. Templates, manifest, rendering, ownership refusal, status, and load live here so a service never has two owners racing to render it. | `agentstart/config/launchd/*.plist`; `agentstart/scripts/install-agent-clis`; `agentstart/scripts/install-launchagents`; `agenthud/scripts/install.sh`; `agentlab/scripts/install.sh`; `agentvoice/scripts/install.sh`; asserted by `agentstart/tests/{install-agent-clis.test.ts,install-launchagents.sh,validate.sh}` |
...
| AgentStart manager/worker roles | agentchats | Use `routing-receipt` through the discovered MCP or documented CLI to validate bounded decision/acceptance tool output; no new store or service. `routing` reads exact native transcripts to join the evidence. Missing capability preserves a capture gap without blocking work. | `agentstart/roles/{manager,worker}/APPEND_SYSTEM_PROMPT.md`; `agentchats/src/routing/`; `agentchats/src/cli/contract.ts` |
...
| wiki | hud, brain, chats | the durable home the others cite into. Durable work state uses HUD; Wiki's `search` is its own subcommand, not the search skill |
| GUIDELINES.md (this repo) | brain, chats, hud, notify, wiki, terminal-control | spliced into linked collab/build/maintain references; applies research reuse and durable Work tracking when relevant, preserves notifications and managed PTY work, and routes durable wiki knowledge while honoring requested artifact formats and destinations. Capability discovery uses skill descriptions |
...
Updated 2026-09-14 for AgentMux's offline-safe role MCP. Both explicit roles
start the fixed `agentmux mcp --instance default` stdio bridge, whose stable
...
| AgentStart notification router | AgentNotify | calls | `config/notifications/terminal-notifier` probes `agentnotify diagnose` with a 12-second bound, then execs AgentNotify with unchanged argv/stdin. If the probe or executable is unavailable, it returns 127 without another submission. `scripts/install-agentlaunch-shims` invokes the internal ownership-checked `install-notification-shim`, which refuses installation while an executable Homebrew terminal-notifier is linked at a configured standard prefix unless it resolves to the managed router or AgentNotify. No Homebrew binary is overwritten. |
...
fxnk-owned Fx Integration installation. AgentLab now owns the reconnectable Fx
ACP broker above; this replaces only the retired AgentFX execution controller
and does not restore its manager MCP, worker roster, routing context, or HUD
observer.
