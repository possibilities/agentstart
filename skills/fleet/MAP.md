...
flowchart LR
    ...
    start -.->|config/launchd + install-launchagents| services[agentbrain work + share + doctor / agentusage observe / agentattention serve / agenthud serve / agentlab serve / agentscrape process-queue / agentsource receive + notify / agentwiki serve]
```
...
| agentstart | agentlab | `install-agent-clis` invokes AgentLab's hardened `scripts/install.sh --install` contract: frozen npm dependencies, cumulative client/server build, ownership-checked editable `agentlab` command, and deployed-SHA receipt without a service action. AgentStart separately owns the exact `io.arthack.agentlab.serve` lifecycle. | `agentstart/scripts/install-agent-clis`; `agentlab/scripts/install.sh`; `agentlab/scripts/install.mjs`; `agentstart/docs/adr/0033-supervise-agentlab-cumulative-service.md` |
