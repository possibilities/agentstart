...
| agentstart | Claude Code / Codex | `install-harness-shims` installs PATH shims that only add default unattended permissions; utility and explicitly configured invocations pass through | `agentstart/scripts/install-harness-shims`; `agentstart/tests/validate.sh` |
