...
| --- | --- | --- | --- |
| agentvoice serve | portless | Locked 0.15.6 foreground route `https://agentvoice.localhost`, editable Vite dev by default with optional production preview. Requires the existing shared loopback HTTPS proxy; no sudo, tunnels or call startup. AgentStart keeps it resident as `io.arthack.agentvoice.serve`; exact-label convergence can replace the temporary submitted reader and later diagnose or converge it without operating AgentVoice's separately owned waiting server, menu app, clients, calls or future Native SDK shell. | `agentvoice/src/web-serve.ts`; `agentvoice/web/server/dev.ts`; `agentvoice/web/server/preview.ts`; `agentvoice/web/README.md`; `agentstart/config/launchd/io.arthack.agentvoice.serve.plist`; `agentstart/scripts/install-launchagents`; `agentstart/docs/adr/0022-supervise-agentvoice-transcript-reader.md` |
| agentvoice test serve | portless | Bounded interim route `https://agentvoice-test.localhost`, executed from the fixed parallel-test checkout and pinned to `~/.local/state/agentvoice/test-workspace`. AgentStart keeps the named reader resident as `io.arthack.agentvoice-test.serve` beside the independently supervised `io.arthack.agentvoice-test.wait`. The reader stays offline when that exact workspace socket is absent; it never falls back to the default reader/server. | `agentvoice/docs/parallel-test-environment.md`; `agentvoice/src/web-serve.ts`; `agentvoice/web/server/live-reader.ts`; `agentstart/config/launchd/io.arthack.agentvoice-test.wait.plist`; `agentstart/config/launchd/io.arthack.agentvoice-test.serve.plist`; `agentstart/scripts/install-launchagents`; `agentstart/docs/adr/0023-supervise-replaceable-agentvoice-test-services.md` |
