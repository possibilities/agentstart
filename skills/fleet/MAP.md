...
| AgentStart | AgentFX | Calls the checkout-owned `scripts/install.sh --install` after AgentUsage. Installs the editable command without a service or active-call restart. The manager role, and only that role, loads the four-tool MCP against the private quota-routing config; bounded `run --config --file` shares the same durable ledger and idempotency fence. Existing loaded managers retain their prior inventory until a later normal role load. | `agentstart/scripts/install-agent-clis`; `agentstart/roles/{manager,worker}/mcp.json`; `agentstart/docs/adr/0028-expose-agentfx-to-managers.md`; `agentfx/scripts/install.sh`; `agentfx/src/{main,mcp}.ts` |
...
| AgentVoice | AgentUsage | On exact native runtime startup, settings changes and a coalesced five-minute refresh, reads `routing evidence --json` and invokes the pure `routing compose-native --file - --json` seam. Uses the owned native model/list catalog; never reads credentials or assumes native account correlation. Numeric economics remain unavailable. | `agentvoice/src/core/routing-orientation.ts`; `agentusage/src/routing-context/native-manager.ts`; `agentusage/src/cli.ts` |
| AgentVoice | AgentHUD | `routing state/apply` stores generation/revision/digest-fenced context and reads full delivery plans. One named `agentusage.routing_context` native output per persisted revision carries silent background facts; transport acceptance never writes a consumed receipt. | `agentvoice/src/core/routing-orientation.ts`; `agentvoice/docs/adr/0090-deliver-manager-routing-orientation.md`; `agenthud/src/routing-cli.ts` |
