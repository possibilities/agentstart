...
| agentstart | AgentVoice default role | the existing fixed-resource renderer publishes a standard role linking the app-owned default prompt files, common portable skills, and existing MCP resources. AgentVoice loads that role through process-local skill roots and per-thread MCP config; source prompt bytes, native delegation, independent roles, and active calls remain under their existing owners | `agentstart/scripts/render-roles`; `agentstart/scripts/render-capabilities`; `agentstart/config/agentvoice/server.json`; `agentstart/tests/render-agentvoice-role.py`; `agentvoice/src/core/role.ts`; `agentvoice/src/core/runtime.ts`; `agentvoice/src/core/params.ts` |
