...
flowchart LR
    ...
    collab -->|Hub invokes agent_send for attached Event Messages| mux
```
...
| agentcollab | agentmux | when `collab_attach` names an Agent, the Sheet sends each matching human Event through its Hub as one `mcp__agentmux__agent_send` call with `{ name, message }`. `COLLAB_SEND_TOOL` can substitute another tool with that shape; without a reachable tool the Event stays pending in `collab_events`. Renaming or changing `agent_send` breaks automatic Event delivery while the MCP notification stream remains independent | `agentcollab/src/server.ts` (`DEFAULT_SEND_TOOL`, `detectMessaging`, `Collab.sendEvent`); behavioral coverage in `agentcollab/test/server.test.ts`; callee contract in `agentmux/src/protocol.ts` (`agent.send`) |
...
Updated 2026-09-03 for agentcollab's explicit agentmux dependency: attached
Sheet Events invoke `agent_send` through the Hub, while an unavailable Message
