...
| agentstart | OpenCode 2 | Converges `@opencode/cli@2.0.16` in a marked private npm prefix, publishes `opencode` into `~/.local/bin`, and retires only its owned `opencode2` link. Existing V1 processes and user data remain untouched. | `agentstart/scripts/install-opencode`; `agentstart/scripts/install.sh`; `agentstart/docs/adr/0044-cut-over-opencode-command-to-v2.md` |
