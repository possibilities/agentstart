...
| AgentVoice | AgentUsage | On exact native runtime startup, settings changes and a coalesced five-minute refresh, reads `routing evidence --json`, exact-revision `routing grok-catalog --json`, and the pure `routing compose-native --file - --json` seam. The named native output carries the complete sanitized Grok catalog, explicit dispositions, reviewed/routable intersection and structured drift beside the owned Codex model/list catalog and quota. It never reads credentials or assumes native account correlation; cross-provider economics remain unavailable. | `agentvoice/src/core/routing-orientation.ts`; `agentusage/src/{grok/catalog,routing-evidence/grok-catalog,routing-context/native-manager}.ts`; `agentusage/src/cli.ts` |
