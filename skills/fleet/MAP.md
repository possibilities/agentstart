...
flowchart LR
    ...
    roles -->|opencode: require V2; private server and append plugin| opencode
    ...
    claude & codex -->|default managed individual stdio MCPs| contractServers[agentattention / agenthud / agentwiki / agentbrain / agentsearch / agentscrape / agentkeys / agentbrowse / agentgrok / agentsounds / agentchats]
...
flowchart LR
    ...
    start ==>|private pinned npm prefix; opencode on PATH| opencode_v2[OpenCode 2]
    ...
    start ==>|checkout contracts| fleet[agentwiki / archival agentboard / agentbrowse / agentattention / agentutils / agentsearch / agentkeys / agentsource / agentscrape / agentbrain / agentusage / agentgrok / agentchats / agentroles]
...
| agentstack | codexnk | Every `bot_start` launches the fixed `~/.local/libexec/codexnk/codex` on a private Unix WebSocket. AgentStack stores named Codex account credentials in private SQLite and supplies process-local `--identity`, `--capabilities`, and `--history-dir` axes. It reconciles refreshed credentials after exit without switching a running Bot's identity. | `agentstack/packages/bots/src/supervisor.ts`; `agentstack/packages/bots/src/runtime-auth.ts`; `agentstack/docs/adr/0029-bots-own-codex-lifecycle.md` |
| agentstack | OpenCode / Devin CLI ACP | The `workers` child owns one stdio ACP process for each ready account: OpenCode for Grok and Codex, Devin CLI for Devin. It creates per-Worker Git worktrees and durable session/turn records, supplies Role skills and MCPs, and re-reads native account-bound model/effort options. The account profile, not AgentUsage selection, chooses native credentials. Internal MCP URLs carry an exact Worker/runtime proof; the owner exposes only read-only Package API operations to them. `devin acp` bypasses AgentStart's terminal worktree shim. | `agentstack/packages/workers/src/{supervisor,manager,resources,worktree}.ts`; `agentstack/packages/api/src/{mcp,bot-mcp-identity}.ts`; `agentstack/docs/adr/0038-durable-acp-worker-execution.md` |
...
| agentroles | OpenCode 2 | Accepts only `opencode` and requires V2. It translates the role to native config, appends through a per-launch plugin hook, and runs a private `--standalone` server so the role does not reach a shared server or another invocation. Remote-server and utility launches with a nonempty role are refused. No global fleet overlay is applied. | `agentroles/src/main.ts`; `agentroles/src/deliver/opencode.ts`; `agentroles/src/render.ts`; `agentroles/docs/adr/0008-opencode-2-only-role-delivery.md` |
| agentstart | OpenCode 2 | Converges `@opencode/cli@2.0.16` in a marked private npm prefix, publishes `opencode` into `~/.local/bin`, then retires only the verified V1 executable and the owned `opencode2` link. Existing V1 processes and user data remain untouched. | `agentstart/scripts/install-opencode`; `agentstart/scripts/install.sh`; `agentstart/docs/adr/0044-cut-over-opencode-command-to-v2.md`; `agentstart/docs/adr/0046-retire-opencode-v1-executable.md` |
...
| agentstart | Devin CLI | Full installation gets the official native CLI only if its versioned binary is absent. `install-harness-shims` replaces only its exact public native symlink or an AgentStart-owned prior wrapper with an atomic `~/.local/bin/devin` command. New terminal sessions run from an owned worktree whose `.devin` contains the default Role's skills and MCPs plus a user-triggered `/prime`; utilities and ACP pass to the native executable unchanged. Native login and history remain Devin-owned. | `agentstart/scripts/install.sh`; `agentstart/scripts/install-harness-shims`; `agentstart/scripts/devin-worktree.ts`; `agentstart/tests/devin-worktree*.test.ts`; `agentstart/docs/adr/0044-delegate-devin-role-to-worktree.md` |
...
| --- | --- | --- | --- |
| Grok Build | official stable Homebrew cask | installs only the standalone native CLI/TUI; no surface integration | `agentstart/scripts/install.sh`; `agentstart/tests/validate.sh` |
...
| OpenCode 2 | 2.0.16 | default executable `opencode` is pinned to the build empirically checked with AgentRoles' V2 role delivery; the private prefix itself is never added to PATH | `agentstart/scripts/install-opencode`; `agentstart/docs/adr/0044-cut-over-opencode-command-to-v2.md` |
