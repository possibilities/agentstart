...
| Funk notification helpers | AgentStart notification router | calls | `funk/bin/.local/bin/funk-notify`, `funk/bin/.local/bin/dismiss-terminal-notifier`, `funk/libexec/funk-update`, and `funk/libexec/verify-notifications` prefer `~/.local/bin/terminal-notifier`. Diagnosis recognizes AgentNotify and reads its authorization without creating a durable probe task; original-notifier fallback diagnosis remains available. |
...
AgentNotify’s app owns its private account-local Unix socket. No AgentStart LaunchAgent is installed for it; CLI/MCP launches the app on demand. The managed `~/.local/bin/terminal-notifier` router now redirects PATH callers through AgentNotify. Funk explicitly prefers that path even when its caller has Homebrew first. The original Homebrew notifier remains installed and is used if the AgentNotify executable or its read-only readiness check is unavailable. Once a request is dispatched, the router preserves its output/status and never replays it through the fallback. Absolute and vendored binaries bypass PATH routing. Native notification acceptance does not establish banner visibility or a Focus bypass.
