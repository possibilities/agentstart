...
flowchart LR
    ...
    claude -->|managed-session MCP stdio| executor[Executor]
    codex -->|managed-session MCP stdio| executor
    executor -->|stdio tools generated from agent contracts| contractServers[agentboard / agentwiki / agentbrain / agentsearch / agentscrape / agentkeys / agentbrowse / agentgrok]
    executor -->|registered native MCPs| browser & terminal[termctrl]
    executor -->|authenticated connections| gmail[Gmail]
    executor -->|computer-use app-server adapter| codex
    harnesses -->|project-local MCP stdio: component registry| shadcn[shadcn]
...
flowchart LR
    ...
    start ==>|Homebrew cask + vendor-owned service + declared integration registry| executor[Executor]
    ...
    start ==>|checkout contracts| fleet[agentwiki / agentboard / agentbrowse / agentattention / agentutils / agentsearch / agentkeys / agentsource / agentscrape / agentbrain / codex-swap / grok-swap / agentusage / agentlaunch / agentsurface / agentgrok / agentchats / agentroles / peekaboo]
    ...
    launch ==>|native codex-swap run/resume + qualified skill and configured MCP enables| codex
...
flowchart LR
    ...
    subgraph guidanceSkills [agentguidance skills]
        ...
        email -.->|mail through| executor[Executor]
...
| agentstart | Executor | installs or upgrades the official Homebrew cask for its signed CLI, then delegates `sh.executor.daemon` rendering, takeover, and lifecycle to `executor service install`. The supervised server starts at login with AgentStart's fleet-aware `PATH` and serves the shared catalog without the desktop sidecar. AgentStart never renders a competing plist. Its explicit installer converges declared no-auth MCP registrations through the CLI's structured stdio MCP protocol, records owned configurations, and preserves independent integrations/credentials. It journals paused execution IDs and accepts only the exact installer-requested registry confirmation; no global policy changes. Its fixed resources connect managed Claude/Codex sessions to Executor | `agentstart/scripts/install.sh`; `agentstart/scripts/executor-integrations`; `agentstart/config/executor/integrations.json`; `agentstart/config/executor/README.md`; `agentstart/config/resources/mcp-servers.json`; Executor's `service install` contract; `agentstart/tests/executor-integrations.py`; Homebrew cask `executor` |
...
| agentstart | agentlaunch / Codex | builds one fixed private resource tree, renders Claude's session-only `agent` plugin with Executor and project-local shadcn MCP servers and Codex's globally installed strictly skills-only `agent` plugin, and persistently name-disables every qualified Codex skill. The same MCP definitions are also rendered for AgentLaunch's Codex session config; the full installer removes ambient shadcn plus the retired LiveKit MCP and skill while preserving unrelated MCPs. Portable skill manifests stay bare; only the disposable Codex copy qualifies default prompts. The full installer removes AgentStart-managed entries from Fx-visible ambient roots and the ownership-proven retired capability tree, while the six-hour sync remains unattended-safe | `agentstart/scripts/sync-skills`; `agentstart/scripts/render-capabilities`; `agentstart/scripts/install.sh`; `agentstart/scripts/sync-codex-skill-policy`; `agentstart/config/resources/*`; `agentstart/docs/adr/0002-render-one-private-resource-set.md` |
| agentlaunch | Claude Code / Codex | loads the one fixed resource set. Claude receives the synthetic `agent` plugin with skills, Executor, and project-local shadcn; native Codex `run`, `resume`, `exec`, and `review` name-enable every qualified `$agent:<skill>` and inject the same MCP definitions through session config, remaining inside codex-swap's ordinary account pin. Utility and unmanaged harness invocations receive neither resource. AgentLaunch owns no App Server, Unix socket, remote TUI, fake provider, projection, or receipt; native Codex therefore owns linked-worktree trust and the complete session lifecycle | `agentlaunch/src/resources.ts`; `agentlaunch/src/launch.ts`; `agentlaunch/docs/adr/0030-use-fixed-resources-with-native-codex.md` |
| Claude Code / Codex | Executor | managed model sessions launch the signed `executor mcp --no-artifacts --elicitation-mode model` bridge. Native skills remain in each harness; downstream tools are discovered through Executor. Utility invocations receive no fleet resources | `agentstart/config/resources/mcp-servers.json`; `agentstart/scripts/render-capabilities`; `agentlaunch/src/resources.ts`; `agentlaunch/test/resources.test.ts` |
| Executor | agentboard / agentbrain / agentbrowse / agentkeys / agentscrape / agentsearch / agentwiki / agentgrok / agent-browser / termctrl / Codex | the source-owned registry declares their stdio MCP entrypoints and the Codex computer-use app-server adapter. Fleet checkout absence skips registration. Native third-party tool coverage is recorded separately from CLI parity; shared processes require explicit resource/session identity and absolute paths | `agentstart/config/executor/integrations.json`; `agentstart/scripts/executor-integrations`; `agentstart/config/executor/README.md`; each installed server's MCP catalog |
| email skill | Executor Gmail / notify | selects a discovered authenticated Gmail connection by account profile, then searches, reads, drafts, sends, and handles attachments through that connection. Existing send authorization is preserved; uncertain sends are reconciled before retry. Notify reaches the human when authentication blocks unattended work | `agentguidance/skills/email/SKILL.md`; `agentguidance/skills/email/references/messages-and-mime.md`; live Executor `google_gmail` tool schemas |
| Jobsearch | Executor Gmail | the email sync opens a structured stdio MCP session, resolves the intended account, pages changed threads, and reads headers used by reply matching and outcome stamping. Authentication failures retain exit 4; incomplete fetches cannot advance the sync watermark. The client sends no mail and accepts no interaction | `jobsearch/cli/src/email/executor.ts`; `jobsearch/cli/src/email/index.ts`; `jobsearch/cli/test/executor-email.test.ts` |
...
| Claude Code / Codex / Fx | agentboard / agentwiki / agentbrain / agentsearch / agentscrape / agentkeys / agentbrowse / agentgrok | an MCP host starts `<cli> mcp` over stdio and receives tools GENERATED from that CLI's own agent contract — never a hand-written tool list, so a command added to the contract becomes a tool with no further edit. Each server runs inside the CLI process and dispatches through its own command table: no subprocess, no argv round trip. Only `audience: agent` leaves are exposed, which makes the CLI's owner — not the consumer — the one who decides what an agent may call. The mapping is fixed once in `agentstart/config/agent-contract/MCP.md` rather than invented eight times | `agentstart/config/agent-contract/MCP.md`; `agentboard/src/mcp-tools.ts` (the reference the other six copy); each repository's `src/mcp.ts`, `src/mcp-server.ts`, `src/mcp-tools.ts` and its stdio handshake test |
...
| --- | --- | --- | --- |
| `~/source/realiti4--claude-swap` | `integration` | `cswax` via `/maintain` and `scripts/install.sh --install`, called by `agentusage/scripts/install-providers.sh` | all three of upstream's CI jobs (Ubuntu, macOS, macOS keychain contract), plus the fork's CI green on the exact candidate |
...
Codex-swap no longer binds a codex-multi-auth Clone: it uses the exact stock
...
desktop sidecar, owns the catalog and captures the fleet command path. Managed
Claude/Codex sessions now connect through the fixed resource definitions; the
explicit full installer converges the source-owned MCP registration set while
content/scheduled skill syncs leave the shared catalog untouched.
