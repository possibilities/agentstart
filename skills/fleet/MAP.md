...
flowchart LR
    ...
    start -.->|config/launchd + install-launchagents| services[agentbrain work + share + doctor / agentusage observe / agentattention serve / agenthud serve / agentlab codex-app-server + serve / agentscrape process-queue / agentsource receive + notify / agentwiki serve]
```
...
| agentstart | agentlab | `install-agent-clis` invokes AgentLab's hardened `scripts/install.sh --install` contract: frozen npm dependencies, cumulative client/server build, ownership-checked editable `agentlab` command, and deployed-SHA receipt without a service action. AgentStart separately owns and orders the dedicated `io.arthack.agentlab.codex-app-server` before `io.arthack.agentlab.serve`, rendering the daemon's exact installed `unix://` endpoint only into the server environment. Socket overrides select desired state only during daemon convergence; console-only convergence and status observe the owned installed plist. | `agentstart/scripts/install-agent-clis`; `agentstart/config/launchd/io.arthack.agentlab.{codex-app-server,serve}.plist`; `agentstart/scripts/install-launchagents`; `agentlab/scripts/install.sh`; `agentlab/scripts/install.mjs`; `agentstart/docs/adr/{0033-supervise-agentlab-cumulative-service,0034-dedicated-agentlab-codex-app-server}.md` |
